...
# Drop NA target rows
model_df = merged_df.dropna(subset=['next_return']).sort_values('time_bin')
...
for fold, (train_idx, test_idx) in enumerate(tscv.split(X), 1):
    ...
    print(f"Fold {fold} — RMSE: {rmse:.6f}, R²: {r2:.4f}, Directional Acc: {direction_acc:.4f}")

plot_importance(model)
plt.title("Feature Importance (Last Fold Model)")
plt.show()

# Final hold-out evaluation (80/20 split)
...
final_model = XGBRegressor(n_estimators=1000, learning_rate=0.05, max_depth=3, random_state=42)
...
direction_acc_final = np.mean((y_pred_final > 0) == (y_test_final > 0))

print("\n--- Final 80/20 Split Evaluation ---")
print(f"Final RMSE: {rmse_final:.6f}, R²: {r2_final:.4f}, Directional Acc: {direction_acc_final:.4f}")
...
results_df['predicted_next_return'] = y_pred_final

results_df[['symbol', 'time_bin', 'predicted_next_return']].to_csv("predictions_numerical.csv", index=False)

print("Final predictions saved to 'predictions_numerical.csv'.")
